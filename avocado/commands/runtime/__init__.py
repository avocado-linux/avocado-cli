...
class RuntimeCommand(BaseCommand):
    ...
    def register_subparser(cls, subparsers):
        ...
        runtime_parser = subparsers.add_parser("runtime", help="Runtime management commands")
    ...
    def execute(self, args, parser=None):
        ...
        if not hasattr(args, 'runtime_subcommand') or args.runtime_subcommand is None:
            if parser:
                ...
                print("Available runtime subcommands: 'build'", file=sys.stderr)
            return False
        ...
        if args.runtime_subcommand == "build":
            ...
            print(f"Unknown runtime subcommand: {args.runtime_subcommand}", file=sys.stderr)
            if parser:
                parser.print_help() 
            return False
