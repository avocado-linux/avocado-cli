...
class RuntimeCommand(BaseCommand):
    ...
    def register_subparser(cls, subparsers):
        ...
        build_parser = RuntimeBuildCommand.register_subparser(
            runtime_subparsers)
    ...
    def execute(self, args, parser=None, unknown=None):
        ...
        if not hasattr(args, 'runtime_subcommand') or args.runtime_subcommand is None:
            if parser:
                ...
                print("Available runtime subcommands: 'build', 'list', 'deps'",
                      file=sys.stderr)
            return False
        ...
        if args.runtime_subcommand == "build":
            ...
            print(f"Unknown runtime subcommand: {
                  args.runtime_subcommand}", file=sys.stderr)
            if parser:
                parser.print_help()
            return False
