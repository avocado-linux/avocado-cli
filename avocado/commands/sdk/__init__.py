...
from .install import SdkInstallCommand  # Added install command
    ...
    def execute(self, args, parser=None):
        ...
        if not hasattr(args, 'sdk_subcommand') or args.sdk_subcommand is None:
            if parser:
                ...
                print("Available SDK subcommands: 'run', 'install', 'compile'", file=sys.stderr)
            return False
        ...
        if args.sdk_subcommand == "run":
            ...
            print(f"Unknown SDK subcommand: {args.sdk_subcommand}", file=sys.stderr)
            if parser:
                parser.print_help() 
            return False
