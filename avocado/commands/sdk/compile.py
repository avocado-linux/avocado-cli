...
class SdkCompileCommand(BaseCommand):
    ...
    def execute(self, args, parser=None, unknown=None):
        ...
        if sections:
            ...
            if missing_sections:
                print_error(f"The following compile sections were not found: {
        ...
        for section in compile_sections:
            ...
            if success:
                ...
                print_error(f"Failed to compile section '{
                    section_name}'.")
                overall_success = False

        if overall_success:
            print_success(f"All {len(compile_sections)
