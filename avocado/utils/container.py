...
class ContainerRunner:
    ...
    def run_container_command(
        ...
        try:
            ...
            print(f"Container execution failed: {e}", file=sys.stderr)
            return False
    ...
    def _execute_container_command(self, container_cmd: List[str], detach: bool = False) -> bool:
        ...
        try:
            ...
            print(f"Error: {
                  self.container_tool} command not found. Is it installed and in your PATH?", file=sys.stderr)
            return False
...
class SdkContainerHelper:
    ...
    def _create_entrypoint_script(self, target: str) -> str:
...
if [ -f "${{AVOCADO_SDK_PREFIX}}/environment-setup" ]; then
    echo "Found ${{AVOCADO_SDK_PREFIX}}/environment-setup"
else
    ...
    # Before calling DNF $AVOCADO_SDK_PREFIX/usr/lib/rpm/macros
    ...
    echo "Setting up sysext|confext sysroots"
...
echo "Changing working directory to /opt"
cd /opt
...
'''
    ...
    def run_dnf_install(
        ...
        entrypoint_script = self._create_entrypoint_script(target)
    ...
    def run_user_command(
        ...
        escaped_command = ' '.join(f'"{arg}"' for arg in command)
