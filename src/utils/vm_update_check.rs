...
pub enum VmUpdateStatus {
    ...
    CliTooOld { message: String, remote: String },
...
mod tests {
    ...
    fn a_newer_release_this_cli_cannot_install_is_reported_not_swallowed() {
        ...
        let VmUpdateStatus::CliTooOld { message, remote } = status else {
        ...
        // Carried separately so `--check --output json` can report the
        // blocked release as a field, not just inside prose.
        assert_eq!(remote, "0.4.0");
    }
